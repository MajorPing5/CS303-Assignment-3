...
const int Infix_To_Postfix::PRECEDENCE[] = { 1, 1, 2, 2, 2, 3, 3, 3, 3, 3, 3 };
...
string Infix_To_Postfix::convert(const string& expression) {
	...
	while (infix_tokens >> next_token) {
		if (isalnum(next_token[0])) {								// Only Alphabet & Numbers values count
			postfix += next_token;
			postfix += " ";
		} else if(is_operator(next_token[0])) {						// Only +, -, /, *, and all parentheses count as operators
...
//	Potential mix-up between Lines 60-64 & 66-72. Might have them mixed up?
//	When reaching ')', it pushes onto stack rather than popping everything and
//	inserting all operators popped (minus parenthesis pair) into postfix string
void Infix_To_Postfix::process_operator(char op) {
	if (operator_stack.empty() || (op == '(')) {
		...
		if (precedence(op) > precedence(operator_stack.top())) {
			...
			// Assesses if Operator stack is empty or if current Operator precedence > top of stack operator precedence;
			operator_stack.push(op);
			if (op == ')') {
